	...
	if created:    
		...
		if count == IS_FLAGGED_COUNT: 
			notify.send(
				...
				recipient=poster, 
				verb=_(
					"Some users considered this post inappropriate. Edit or delete it now to prevent suspension. Also, ensure to avoid posting such posts in future."
				),
				target=post,
			...
			for moderator in User.moderators.all():
				notify.send(
					...
					recipient=moderator, 
					verb=_("This post has been FLAGGED. You can delete it if you find it inappropriate or absolve if you find it appropriate."),
					target=post,
					category=Notification.REPORTED,
					# mark flag as not yet absolved
					absolved=False  
				)
		...
		if count == 2 * IS_FLAGGED_COUNT:
			User.objects.penalize_user(poster, post)
			return
		...
		if count > 2 * IS_FLAGGED_COUNT:
			...
			notify.send(
				sender=poster,  # just use same user as sender
				recipient=poster, 
				verb=_("One of your posts has been deleted because it was considered inappropriate by many users."),
				category=Notification.FLAG
			)
