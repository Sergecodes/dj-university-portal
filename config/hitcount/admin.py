...
from .models import HitCount, Hit, BlacklistedIP, BlacklistedUserAgent
...
class HitAdmin(admin.ModelAdmin):
    ...
    def blacklist_ips(self, request, queryset):
        for obj in queryset:
            ip, created = BlacklistedIP.objects.get_or_create(ip=obj.ip)
            if created:
                ip.save()
    ...
    def blacklist_user_agents(self, request, queryset):
        for obj in queryset:
            ua, created = BlacklistedUserAgent.objects.get_or_create(
...
class HitCountAdmin(admin.ModelAdmin):
    list_display = ('content_object', 'num_of_hits', 'modified')
    fields = ('num_of_hits',)
