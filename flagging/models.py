...
class Flag(models.Model):
    ...
    def is_flagged(self):
        ...
        Note that the flags state is updated when toggle_flagged_state() is called, and the latter is called after every new FlagInstance... """
        return self.state != self.State.UNFLAGGED.value


class FlagInstance(models.Model):
    # copy FLAG_REASONS list so as not to modify the referenced object
    REASON = FLAG_REASONS.copy()
