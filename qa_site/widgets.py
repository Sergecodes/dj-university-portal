...
from taggit_selectize.widgets import bool_or_str, TagSelectize as BaseTagSelectize


class TagSelectize(BaseTagSelectize):
   """Override default options that are unmodifiable"""
   def render(self, name, value, attrs=None, renderer=None):
      ...
      js = u"""
         <script type="text/javascript">
            // Make the select variable accessible in other scripts
            var $select;
            (function($) {
               $(document).ready(function() {
                  $select = $("input[id=%(id)s]").selectize({
                     valueField: 'name',
                     labelField: 'name',
                     searchField: 'name',
                     ...
                     createFilter: function(text) {
                        console.log('in createFilter', text);
                     },
                     plugins: [%(plugins)s],
                     render: {
